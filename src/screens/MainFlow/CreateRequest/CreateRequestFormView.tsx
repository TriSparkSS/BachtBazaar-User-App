...
type CreateRequestFormViewProps = {
  ...
  showHeader?: boolean;
  editingRequest?: BestRequestData | null;
...
const CreateRequestFormView: React.FC<CreateRequestFormViewProps> = ({
  ...
  const mapTimeframeToUrgency = (timeframe?: string): RequestUrgency => {
    const value = String(timeframe ?? '').trim().toLowerCase();
    if (value.includes('today')) {
      return 'today';
    }
    if (value.includes('2') || value.includes('soon')) {
      return 'soon';
    }
    return 'flexible';
  };

  const [product, setProduct] = useState(editingRequest?.title ?? initialProduct?.trim() ?? '');
  const [categoryId, setCategoryId] = useState(editingRequest?.categoryId ?? '');
  ...
  const [location, setLocation] = useState(
    editingRequest?.formattedAddress?.trim() || initialLocation,
  );
            ...
            <TouchableOpacity
              ...
              {isSubmitting ? (
                ...
                <Text style={styles.ctaText}>
                  {editingRequest ? 'Update Request' : 'Get Best Offers'}
                </Text>
              )}
            </TouchableOpacity>
