...
const CreateRequestResultsView: React.FC<CreateRequestResultsViewProps> = ({
  ...
  const handleCall = async (offer: CreateRequestShopOffer) => {
    try {
      await openPhoneDialer(offer.phone);
    } catch (error) {
      showAppAlert(
        'Call unavailable',
        error instanceof Error ? error.message : 'Phone number is not available for this shop.',
      );
    }
  };

  const handleChat = async (offer: CreateRequestShopOffer) => {
    try {
      await openChatWithNumber(
        offer.phone,
        `Hi, I saw your offer on Bachat Bazaar for "${product}".`,
      );
    } catch (error) {
      showAppAlert(
        'Chat unavailable',
