...
const isBufferLike = (value: unknown): value is Record<string, unknown> => {
  ...
  // Nested Mongo binary: { data: { type: 'Buffer', data: [...] }, contentType }
  const nested = record.data;
  return Boolean(
    nested &&
      typeof nested === 'object' &&
      !Array.isArray(nested) &&
      (nested as Record<string, unknown>).type === 'Buffer' &&
...
const redactForLogs = (value: unknown, key?: string): unknown => {
  ...
  if (isBufferLike(value)) {
    const record = value as Record<string, unknown>;
    const nested = record.data;
    const byteLength = Array.isArray(record.data)
      ? record.data.length
      : nested &&
          typeof nested === 'object' &&
          !Array.isArray(nested) &&
          Array.isArray((nested as Record<string, unknown>).data)
        ? ((nested as Record<string, unknown>).data as unknown[]).length
        : 0;
    return {
      type: 'Buffer',
  ...
  if (Array.isArray(value)) {
    // Cap large numeric arrays (raw Buffer.data) so log walks cannot hang.
